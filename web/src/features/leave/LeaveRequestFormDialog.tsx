...
import { useCreateLeaveRequest, useLeaveTypes } from './api';
...
/** Sheet pengajuan izin baru — jenis, rentang tanggal, alasan, lampiran opsional. */
export function LeaveRequestFormDialog({ open, onClose, onSubmitted }: LeaveRequestFormDialogProps) {
  ...
  useEffect(() => {
    if (types && types.length > 0 && !type) setType(types[0].key);
  }, [types, type]);
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Ajukan Izin">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <Field label="Jenis izin" htmlFor="leave-type">
          <Select
            id="leave-type"
            value={type}
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Tanggal mulai" htmlFor="leave-date-start">
            <Input
              id="leave-date-start"
              type="date"
              value={dateStart}
          ...
          <Field label="Tanggal selesai" htmlFor="leave-date-end">
            <Input
              id="leave-date-end"
              type="date"
              value={dateEnd}
        ...
        <Field label="Alasan" htmlFor="leave-reason">
          <Textarea
            id="leave-reason"
            value={reason}
        ...
        <Field label="Lampiran (opsional)" htmlFor="leave-attachment" error={attachmentError ?? undefined} hint={!attachmentError ? 'PDF, JPG, atau PNG — maksimal 5 MB.' : undefined}>
          {attachment ? (
