...
import { useCancelStudentLeaveRequest, useStudentLeaveRequests } from './api';
import { StudentLeaveDetailBody } from './StudentLeaveDetailBody';

const CANCELABLE_STATUSES = new Set(['pending_homeroom', 'pending_bk']);

/** /izin-saya/:id — detail satu pengajuan izin (siswa: milik sendiri; ortu: milik anaknya) + aksi batalkan bila masih pending. */
export function StudentLeaveDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { showToast } = useToast();
  const { data: me } = useMe();
  const { data: items, isLoading, isError, refetch } = useStudentLeaveRequests('mine');
  const cancelRequest = useCancelStudentLeaveRequest();
  ...
  return (
    <div className="flex min-h-dvh flex-col">
      <AppBar title="Detail Izin" onBack={() => navigate('/izin-saya')} />
      ...
      <div className="mx-auto w-full max-w-[640px] flex-1 px-5 py-5">
        {isLoading ? (
          ...
          <div className="flex flex-col gap-6">
            <StudentLeaveDetailBody request={request} showStudent={me?.role === 'orang_tua'} />

            {CANCELABLE_STATUSES.has(request.status) && (
