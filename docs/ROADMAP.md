...
## Fase 16 — Perombakan UI admin & super admin ✅ (kode selesai — menunggu cek visual USER)
- ✅ AppShell: sidebar & top bar STICKY — fix diterapkan: wrapper `lg:h-dvh lg:overflow-hidden`, sidebar `lg:sticky lg:top-0 lg:h-dvh lg:overflow-y-auto`, area konten `lg:h-dvh lg:flex-1 lg:overflow-y-auto lg:min-w-0`, `DesktopTopBar` dibungkus `sticky top-0 z-20 bg-bg` di dalam area konten. Mobile tidak berubah.
- ✅ Hapus batas lebar konten di layar admin/super admin — helper baru `web/src/components/ui/page.ts` (`PAGE_WIDE`/`PAGE_NARROW`), docs/10 §5 direvisi (640px→720px hanya utk form/detail personal). 18 halaman diubah ke `PAGE_WIDE`: teaching/MonitoringPage, settings/SettingsPage, grading/GradingPage, rolePermissions/RolePermissionsPage, studentleave/StudentLeaveAdminLayout, discipline/DisciplinePage, discipline/DisciplineSettingsPage, counseling/CounselingPage, substitution/SubstitutionPage, latearrival/LateArrivalRecapPage, data/DataLayout, announcements/AnnouncementsPage, admin/PlatformAnnouncementsPage, admin/InterestLeadsPage, admin/PlansPage, attendance/AttendanceClassesPage, leave/LeaveRecapPage, billing/BillingPage. Halaman personal/detail/form (profil, izin-saya, nilai-saya, checkin, detail siswa, wizard, dsb) sengaja dibiarkan narrow. Halaman terlarang (dashboard, DataTable-target: teachers/employees/classes/subjects, admin/OutboxPage, admin/RevenuePage, admin/SchoolsListPage, attendance/AttendanceRecapPage) tidak disentuh — jadi tugas agent lain.
- ✅ Super admin: nav LENGKAP bergrup — `lib/nav.ts#getSidebarNav` — Beranda; Sekolah (daftar, tambah, minat); Langganan (plan & harga, pendapatan); Operasional (outbox, pengumuman platform); Akun (profil).
- ✅ Admin sekolah: nav sidebar desktop LENGKAP bergrup (Akademik: siswa/rombel/guru/pegawai/mapel/jadwal/jam/ruangan/tugas; Kegiatan: absensi/rekap absensi/izin guru/izin siswa/kedisiplinan/nilai/konseling/guru pengganti/monitoring/pengumuman; Sistem: pengaturan/hak akses/tagihan/notifikasi/profil) — mobile tetap 5 tab (`getNavItems` tidak berubah). Kepala sekolah & guru/siswa/ortu/pegawai juga dapat grup sidebar (lihat `getSidebarNav`).
- ✅ Dashboard admin sekolah desktop: `features/dashboard/AdminHomePage.tsx` (baru) — 1 fetch `useTvBoard()` (attendance/teaching/announcements) + `useLeaveApprovals()` (izin guru menunggu) + `useStudentLeaveRequests('all')` (izin siswa pending) + `useDisciplineRecords` (pelanggaran 7 hari) bila hook tersedia. StatTile auto-fit (Hadir hari ini, Belum diabsen, Guru mengajar, Belum masuk kelas, Izin guru menunggu, Pelanggaran 7 hari) + grid 2 kolom "Perlu Tindakan" (kelas belum diabsen/sesi belum finalize/izin guru & siswa menunggu, gabungan; kosong → "Semua beres") vs DataTable "Absensi per Rombel Hari Ini" + baris bawah DataTable "Guru Mengajar Sekarang" & "Pengumuman Aktif". `App.tsx` BerandaPage route: role `admin_sekolah` → `<AdminHomePage>`, role lain tidak berubah. `DashboardAdminPage.tsx` (super admin) dipadatkan: "Perlu Perhatian" jadi 1 DataTable gabungan (Jenis · Sekolah · Detail · aksi "Buka"), "Aktivitas Terakhir" jadi DataTable (+ kolom Status diturunkan dari `attention.schools_grace/readonly` yang sama, bukan endpoint baru), kartu "Jalan Pintas" dihapus (menu sudah di sidebar).
- ✅ DataTable diterapkan ke: `features/teachers/TeachersListPage.tsx`, `features/employees/EmployeesListPage.tsx` (Nama·Email/Username·NIP·Status, aksi Ubah/Masuk Sebagai/Aktifkan-Nonaktifkan — reset password TIDAK ditambah, tidak ada hook per-sekolah untuk admin_sekolah), `features/classes/ClassesListPage.tsx` (Nama·Tingkat·Jurusan·Wali·Jml Siswa), `features/subjects/SubjectsListPage.tsx` (Nama·Kode), `features/duties/DutiesListPage.tsx` (Nama·Untuk·Kapabilitas·Petugas, aksi Ubah/Kelola Petugas/Hapus), `features/schedule/RoomsPage.tsx` (Nama, aksi Lihat QR/Hapus), `features/attendance/AttendanceRecapPage.tsx` (Rombel·Hadir·Terlambat·Izin·Sakit·Alpa·Belum·Status), `features/admin/OutboxPage.tsx` (Event·Sekolah·Pengguna·Channel·Percobaan·Dibuat, aksi Retry), `features/admin/RevenuePage.tsx` (tabel Per Bulan & Per Plan jadi DataTable), `features/admin/SchoolsListPage.tsx` (Nama·Slug·Domain Custom·Status — kolom Langganan dilewati: `GET /api/admin/schools` tidak mengirim status langganan per sekolah). Semua tetap ListRow di mobile (`lg:hidden`/`hidden lg:block`), lebar `PAGE_WIDE` dari `components/ui/page.ts`. `features/data/DataLayout.tsx` ikut dilebarkan (wadah bersama 6 halaman /data di atas). `npm run build` & `npm run lint` hijau, grep anti-slop nol pelanggaran nyata.

## Fase 17 — Fokus UI ⬜ (rencana lengkap: docs/13-ui-plan.md; user = auditor visual utama; sprint S1-S7, 1 agent per sprint, commit per sprint)
- ⬜ S1 Fondasi density + PageHeader/Toolbar/SectionCard/KpiRow + data demo kaya
- ⬜ S2 Dashboard super admin 12 kolom + master-detail sekolah + detail sekolah bertab
- ⬜ S3 Dashboard admin sekolah & kepsek + beranda role lain dipadatkan
- ⬜ S4 Halaman data master-detail + detail siswa bertab
- ⬜ S5 Halaman operasional (rekap/kedisiplinan/izin/monitoring heatmap/pengaturan/nilai spreadsheet)
- ⬜ S6 Landing + AuthLayout
- ⬜ S7 Detail lintas app + cek responsif 3 breakpoint
